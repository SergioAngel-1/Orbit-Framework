...
const PendingApprovalModal: React.FC<PendingApprovalModalProps> = ({ isOpen, onClose, user }) => {
  const { t } = useTranslation('authModals');
  // Log cuando cambia el estado de isOpen
  useEffect(() => {
    logger.info('PendingApprovalModal', 'Estado del modal cambiado', { isOpen, userEmail: user?.email });
  }, [isOpen, user]);
  
  // Si el modal no está abierto, no renderizar nada
  if (!isOpen) {
    logger.info('PendingApprovalModal', 'Modal no mostrado porque isOpen es false');
    return null;
  }
  
  logger.info('PendingApprovalModal', 'Renderizando modal', { isOpen, userEmail: user?.email });
    ...
    <div className="fixed inset-0 z-[9999] overflow-y-auto" style={{ zIndex: 9999 }}>
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        >
          ...
          <button 
            onClick={onClose}
            aria-label={t('pending.closeAriaLabel')}
