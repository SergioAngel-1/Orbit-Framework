/**
 * Datos de ubicaciones de Colombia
 * Departamentos y sus municipios principales
  ...
  departments: [
    {
      name: 'Amazonas',
      capital: 'Leticia',
      municipalities: ['Leticia', 'Puerto Nariño', 'El Encanto', 'La Chorrera', 'La Pedrera', 'La Victoria', 'Mirití-Paraná', 'Puerto Alegría', 'Puerto Arica', 'Puerto Santander', 'Tarapacá']
    },
    {
      name: 'Antioquia',
      capital: 'Medellín',
    ...
    },
    {
      name: 'Sucre',
      capital: 'Sincelejo',
      municipalities: ['Sincelejo', 'Buenavista', 'Caimito', 'Chalán', 'Coloso', 'Corozal', 'Coveñas', 'El Roble', 'Galeras', 'Guaranda', 'La Unión', 'Los Palmitos', 'Majagual', 'Morroa', 'Ovejas', 'Palmito', 'Sampués', 'San Antonio de Palmito', 'San Benito Abad', 'San Juan de Betulia', 'San Luis de Sincé', 'San Marcos', 'San Onofre', 'San Pedro', 'Santiago de Tolú', 'Sucre', 'Tolú Viejo']
    },
    {
      name: 'Tolima',
      capital: 'Ibagué',
...
export const getDepartments = (): string[] => {
  return colombiaData.departments.map(d => d.name).sort();
};
...
export const getMunicipalitiesByDepartment = (departmentName: string): string[] => {
  const department = colombiaData.departments.find(d => d.name === departmentName);
  return department ? department.municipalities.sort() : [];
};

/**
 * Obtiene la capital de un departamento
 */
export const getCapitalByDepartment = (departmentName: string): string | null => {
  const department = colombiaData.departments.find(d => d.name === departmentName);
  return department ? department.capital : null;
};

/**
 * Busca el departamento al que pertenece un municipio
 */
export const getDepartmentByMunicipality = (municipalityName: string): string | null => {
  for (const department of colombiaData.departments) {
    if (department.municipalities.includes(municipalityName)) {
