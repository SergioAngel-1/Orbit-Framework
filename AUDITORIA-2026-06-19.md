...
sobre todo: una **pasarela real**, **entrega de email transaccional fiable**, **pruebas E2E
...
- 🟠 **Email transaccional sin transporte fiable**: WP usa `wp_mail()` (sendmail PHP) por defecto. Sin SMTP/relay (SES/Postmark/Resend) configurado, **los emails de pedido, reset y verificación no se entregarán de forma fiable** en producción (acaban en spam o no salen). Es uno de los mayores gaps reales de go-live. Falta también doc de SPF/DKIM/DMARC.
...
- **C1 (P0)** Configurar **SMTP/relay** (SES/Postmark/Resend) en WordPress (plugin o `wp_mail` override) con credenciales server-only.
- **C2 (P1)** Documentar y verificar **SPF + DKIM + DMARC** del dominio; añadir a `GO-LIVE.md`.
- **C3 (P2)** Plantillas de email revisadas (branding ya existe) + prueba de entrega automatizada en *staging*.
...
1. **B1, B2** (pasarela real + checkout invitado) y **C1** (email) — sin esto no es un e-commerce real.
...
| Email transaccional | (implícito por Woo) | 🟠 **sin transporte SMTP fiable** (gap go-live) |
