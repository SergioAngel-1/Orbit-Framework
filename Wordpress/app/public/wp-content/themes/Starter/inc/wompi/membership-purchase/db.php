 ...
 * 
 * Responsabilidades:
 * - Crear/migrar la tabla starter_membership_purchases
...
/**
 * Nombre de la tabla de compras de membresías
 */
function starter_membership_purchase_table_name() {
    global $wpdb;
    return $wpdb->prefix . 'starter_membership_purchases';
}

/**
 * Crear tabla para compras pendientes de membresías
 * 
 * Se ejecuta en 'init' para garantizar que la tabla exista.
 * Usa dbDelta para creación idempotente.
 */
add_action('init', 'starter_create_membership_purchases_table', 10);
function starter_create_membership_purchases_table() {
    global $wpdb;
    
    $table_name = starter_membership_purchase_table_name();
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") === $table_name) {
        ...
        if (empty($column_exists)) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('[Starter Membership] Columna wc_order_id agregada a tabla existente');
            }
        }
        return;
    }
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('[Starter Membership] Tabla de compras de membresías creada');
    }
}
 ...
 * @return object|null
 */
function starter_membership_purchase_get_by_reference($reference) {
    global $wpdb;
 ...
 * @return object|null
 */
function starter_membership_purchase_get_by_reference_and_user($reference, $user_id) {
    global $wpdb;
    $table = starter_membership_purchase_table_name();
 ...
 * @return object|null
 */
function starter_membership_purchase_get_for_update($reference) {
    global $wpdb;
    $table = starter_membership_purchase_table_name();
 ...
 * @return int|false
 */
function starter_membership_purchase_update($purchase_id, array $data) {
    global $wpdb;
    $table = starter_membership_purchase_table_name();
 ...
 * @param string $error_status  Ej: 'amount_mismatch', 'error_activation'
 * @param string $wompi_tx_id
 */
function starter_membership_purchase_mark_error($purchase_id, $error_status, $wompi_tx_id = '') {
    starter_membership_purchase_update($purchase_id, [
        'status'               => $error_status,
