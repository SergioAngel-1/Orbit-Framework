...
export async function POST(
  ...
  try {
    ...
    if (!verification.valid || !verification.reference) {
      logger.warn({ event: "payments.webhook.invalid", provider: providerId }, "Firma de webhook inválida");
    ...
    if (verification.status === "declined" || verification.status === "voided") {
      ...
      logger.info({ event: "payments.webhook.cancelled", reference: verification.reference }, "Pedido cancelado por webhook");
    ...
    if (verification.status !== "approved") {
      logger.info({ event: "payments.webhook.ignored", reference: verification.reference, status: verification.status }, "Estado de webhook ignorado");
      return NextResponse.json({ received: true, applied: "none" });
    }

    // Idempotencia: si ya estaba pagado, ACK sin reprocesar.
    if (isOrderPaid(order)) {
      logger.info({ event: "payments.webhook.already_paid", reference: verification.reference }, "Pedido ya estaba pagado");
    ...
    if (!paymentMatchesOrder(order, verification.amountMinor, verification.currency)) {
      logger.warn({ event: "payments.webhook.mismatch", reference: verification.reference }, "El importe/moneda no coincide con el pedido");
      return NextResponse.json(
    ...
    logger.info({ event: "payments.webhook.paid", reference: verification.reference }, "Pedido marcado como pagado");
