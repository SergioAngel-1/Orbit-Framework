...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.info({ event: "coupon.apply.success", code: parsed.data.code }, "Cupón aplicado al carrito");
    return NextResponse.json(data);
  } catch (error) {
    logger.error({ event: "coupon.apply.error", err: error instanceof Error ? error.message : error, code: parsed.data.code }, "Error al aplicar cupón");
...
export async function DELETE(request: Request) {
  ...
  if (!parsed.success) {
    logger.warn({ event: "coupon.remove.validation" }, "Código de cupón inválido al eliminar");
  ...
  try {
    ...
    await writeCartToken(cartToken);
    logger.info({ event: "coupon.remove.success", code: parsed.data.code }, "Cupón eliminado del carrito");
    return NextResponse.json(data);
  } catch (error) {
    logger.error({ event: "coupon.remove.error", err: error instanceof Error ? error.message : error, code: parsed.data.code }, "Error al eliminar cupón");
    return handleApiError(error);
  }
}
